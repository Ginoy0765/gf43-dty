...
def _build_zone(cum_start, cum_end, doc_start, doc_end, zone_transition,
    ...
    if zone_transition == 'stepped':
        ...
        while depth - cum > 1e-9 and len(raw) < 100000:
            ...
            cum += doc
            i += 1
    elif zone_transition == 'pctreduce':
        # Equal reduction %: each pass = previous * (1 - R%).  Two anchors:
        #   'first' - pass 1 = zone First DOC (doc_start); shrink by R% each
        #             pass; hold at zone Last DOC (doc_end) as a FLOOR.
        #   'last'  - final cutting pass = zone Last DOC (doc_end); grow the
        #             cuts backward by 1/(1-R%), capped at First DOC (ceiling),
        #             then reverse so the big cuts run first.
        # Tiny trailing sliver merged into its neighbour.  Last pass trimmed
        # to the zone boundary by the common tail below.
        R = max(1.0, min(90.0, float(pct_reduce)))
        rr = 1.0 - R / 100.0
        F = max(1e-6, doc_start)
        L = max(1e-6, doc_end)
        if str(pct_anchor).lower() == 'last':
            seq = []
            c = 0.0
            d = L
            while depth - c > 1e-9 and len(seq) < 100000:
                dd = d if d <= F else F
                if dd > depth - c:
                    dd = depth - c
                seq.append(dd)
                c += dd
                d = d / rr
            if (len(seq) > 1 and seq[-1] < 0.5 * seq[-2]
                    and seq[-1] + seq[-2] <= F + 1e-9):
                t = seq.pop()
                seq[-1] += t
            seq.reverse()
            raw = seq
            ...
            c = 0.0
            d = F
            while depth - c > 1e-9 and len(raw) < 100000:
                dd = d if d >= L else L
                if dd > depth - c:
                    dd = depth - c
                raw.append(dd)
                c += dd
                d = d * rr
            if len(raw) > 1 and raw[-1] < 0.5 * raw[-2]:
                t = raw.pop()
                raw[-1] += t
    else:
...
def pass_schedule(thread_depth, first_doc, zones, zone_transition='linear',
    ...
    for pct, doc in zones:
        ...
        for d, cum in _build_zone(cum_start, cum_end, prev_doc, doc, zone_transition, pct_reduce, pct_anchor):
            out.append((d, cum))
        prev_pct = pct
        prev_doc = doc
    return out
